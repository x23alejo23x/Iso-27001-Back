...
class AdminController {
  ...
  async crearUsuario(req, res) {
    try {
      const { empresa_id, nombre_usuario, correo_electronico, password, rol_id } = req.body;
      if (!empresa_id || !nombre_usuario || !correo_electronico || !password || !rol_id)
        return res.status(400).json({ message: 'Todos los campos son requeridos' });

      if (![2, 3].includes(Number(rol_id)))
        return res.status(400).json({ message: 'Rol inválido. Solo se puede asignar Auditor (2) o Visor (3)' });

      if (password.length < 8 || !/\d/.test(password))
        return res.status(400).json({ message: 'La contraseña debe tener mínimo 8 caracteres y al menos 1 número' });

      const usuario = await this.adminService.crearUsuario({
        nombre_usuario, correo_electronico, password, rol_id: Number(rol_id), empresa_id,
      });
